...
class MoveToMarkerWithOMPL:
    ...
    def __init__(self):
        ...
        self.x_offset = 0.213  # Adjust this value as needed
    ...
    def run(self):
        while not rospy.is_shutdown():
            if self.transformed_pose:
                ...
                user_input = input("Do you want to continue to the next move? (yes to continue, no to stop): ").strip().lower()
                if user_input != "yes":
                    rospy.loginfo("User chose to stop. Exiting the loop.")
                    break
